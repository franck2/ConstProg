...
public:
	Noeud();
	Noeud(unsigned short int val);
	Noeud copie();
	//deux methodes d'affichage
	void toString();
	void toString2();
	std::vector<std::set<unsigned short int> > getDomains();
	void setDomains(std::vector<std::set<unsigned short int> > domains);
	//supprime les elements d'un ensemble a la position i pour y placer la valeur val
	void clear_and_add(int pos, unsigned short int val);
	//supprime une valeur dans un ensemble a la position i
	void supprimer(int pos, unsigned short int val);
